...
S.P.I.D.E.R. runs inside **Claude Code**, a tool you drive by typing in plain English. You don't need
...
Option B (terminal): `git clone https://github.com/koushik1610/spider.git`
...
**Download it and unzip it.** Inside is a folder full of `.csv` files — that folder is what SPIDER
reads. You can start SPIDER before it arrives; it'll do a lighter first pass and you add the export
later.

## Step 4 — Find your folder paths (the one fiddly bit)
SPIDER will ask "where is your LinkedIn export folder?" and "where is your resume?" It wants the
...
You don't have to get this perfect — if a path is wrong, SPIDER will tell you and ask again.

## Step 5 — Start it
In your terminal, go into the unzipped `spider` folder and run `claude`. Then type:

```
/spider
```

…or just say **"Run SPIDER."** Answer its questions (your name, those two folder paths, the jobs you
...
- "I want to try it cheaply first" → tell it: *"Run SPIDER but only Phase 1"* (just the LinkedIn
