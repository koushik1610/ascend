# Ascend vs. career-ops (+ apply-factory): gap analysis and adoption plan

**Date:** 2026-08-20 · **Ascend at:** v0.6.0 + unreleased run-council work on `main`
**Compared against:** [`santifer/career-ops`](https://github.com/santifer/career-ops) (`8f3bcf3`) and
[`itshoax/career-ops-extension`](https://github.com/itshoax/career-ops-extension) (apply-factory).

This is a read of two mature peer systems against ours, a list of what Ascend is genuinely missing,
and a tiered plan for what to take. It is analysis only. Nothing here is a scope-freeze exception:
see [Sequencing against the scope freeze](#sequencing-against-the-scope-freeze).
...
| **career-ops** | A local-first, AI-agnostic career operating system | **~70 Node scripts, 40 modes, 80+ job-board providers, ~1,100 files.** Data-driven. Scripts compute, the agent narrates. |
| **apply-factory** | A form-filling and answer-learning extension for career-ops | **6 modes, ~10 Python files, one SQLite KB.** A learning loop bolted onto someone else's pipeline. |

### The structural difference that explains everything else

**Ascend produces artifacts. career-ops accumulates evidence.**

Ascend's durable state is `.ascend-state.json` plus a tree of Markdown the agent re-reads and
re-reasons over. career-ops's durable state is a declared **data contract**: a canonical tracker
...
Everything career-ops has that Ascend doesn't — funnel velocity, rejection-pattern detection,
...
The second structural difference: **career-ops does an enormous amount of work at zero tokens.**
...
- **Cold start.** Ascend interviews you and builds the master résumé from a LinkedIn export.
  career-ops largely assumes you arrive with a finished `cv.md`; its `intake.mjs` is a newer,
...
- **Lock the master + selection-only.** `master_locked` / `master_version` in `.ascend-state.json`
  ...
  guideline. career-ops has strong source-of-truth rules in `_shared.md` but no lock/version
...
- **MASTER GAP.** Naming the missing-bullet case and routing it back to the master, rather than
  letting the derivative invent, is a better-designed honesty primitive than anything in career-ops.
- **The renderer fails instead of shipping.** `tools/render_resume.py` enforcing a one-page budget
  and erroring on overflow is stricter than career-ops's PDF path.
- **The warm-network layer.** Phase 11 mining `Connections.csv` + `messages.csv` for real DM warmth
  is genuinely unique. career-ops's `contacts.tsv` is a phonebook you fill in by hand.
- **Visual entry points.** `linkedin-analysis.html`, `start-here.html`, the résumé builder, and
  `/ascendui` have no career-ops equivalent in the core (their web UI is a separate Next.js app; the
  Go TUI is optional and isolated).
- **Leanness.** ~25–30 files on a first run against career-ops's 70-script flat root. That is a real
...
| # | Missing | career-ops equivalent | Consequence for Ascend |
|---|---|---|---|
...
Ascend is strong from intake through "send the application" and thin after. career-ops has an entire
...
career-ops ships five jurisdiction data tables under `templates/`:
...
### G. From apply-factory specifically
...
| # | Missing | apply-factory equivalent |
|---|---|---|
...
- **The flat 70-script root.** career-ops defends it on path-stability grounds — a legitimate
...
- **Hardcoded model names in prompts.** career-ops's `_shared.md` spend-tier table names specific
  models and openly concedes it can't keep the non-Claude rows current. Adopt the *tier concept*,
  not the table.
- **SQLite as a primary store.** career-ops settled this correctly (files canonical, DB derived) and
...
**Honesty note:** append-only ledgers are a *better* fit for Ascend's values than for career-ops's.
...
- **Provenance on every entry.** Tag each answer `verified` (traces to master résumé / intake /
  ...
  applied to the KB.
- **Never overwrite, always version.** apply-factory's own roadmap regrets that `learn` overwrites
...
broad seed. Keep career-ops's three disciplines exactly: citable regulator-grade source, effective
...
This is the one item on this list that would make Ascend *better than* career-ops at something
career-ops invented, because it lands inside an honesty framework that is already the product's
stated identity.
...
career-ops is roughly two orders of magnitude more code and has a large contributor base. Most of
...
1. **The state layer** (Tier 1) — without it, the entire right half of the funnel stays
   unbuildable.
2. **The learning loop** (Tier 2) — the one idea in apply-factory worth taking wholesale, and it
